...
for(i in 1:100){
  ...
  write.csv(fmm.ls1.looic ,"../results/loo.littersize1.looped.csv" )
  
  # extract samples
  ls1.samplesi<- posterior_samples(global.fmm.ls1i)
  post.ls1.looped<- rbind(ls1.samples1 , ls1.samplesi)
  save(post.ls1.looped, file="../results/post.ls1.looped.robj")
...
sample_prior = "only" and then creating prior predictive plots via pp_check.
```{r}
prior.int.ls1.fmm <- brms::get_prior(rts ~  litter_year * females_mate_multiply +
                                  longevity + 
            (1|gr(Binomial, cov = A[[1]])) +  (1|phylo), 
          data = dfls1, 
          data2 = list(A = A[[1]]),
...
m.int.ls1.fmm <- brm(rts ~  litter_year * females_mate_multiply +
  ...
  family = lognormal(), 
  data2 = list(A = A[[1]]),
  prior = prior.int.ls1.fmm, #c(prior(student_t(3, 0, 2.5))) 
  chains = 4, cores = mycores, # sample_prior = TRUE / "only"
  iter = 5000, warmup = 3000,  # iter = 10000, warmup = 1500,
  control = list(adapt_delta = 0.99, max_treedepth = 15)
  #, save_pars = save_pars(all = TRUE)
...
# make empty data frame to put data in
## loo
ls1.looic  <- data.frame(runby=character(),
                            looic=numeric())
## posterior samples
...
for(i in 1:100){
  ...
  pool <- cbind.data.frame(runby, 
                           looic)
  int.ls1.looic <-rbind(looic , pool )
  write.csv(int.ls1.looic ,"../results/loo.littersize1.interaction.looped.csv" )
  
  # extract samples
  int.ls1.samplesi<- posterior_samples(int.ls1i)
  post.int.ls1.looped<- rbind(int.ls1.samples1 , int.ls1.samplesi)
  save(post.int.ls1.looped, file="../results/post.ls1.interaction.looped.robj")
